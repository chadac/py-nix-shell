...
from nix_shell.expr import FileSet, NixExpr, NixExprType, StorePath, raw
...
def test_fileset_snapshot(snapshot):
    ...
    snapshot.assert_match(expr.dumps(fileset), "fileset_root_files.nix")
...
def test_fileset_duplicate_directories_snapshot(snapshot):
    ...
    snapshot.assert_match(expr.dumps(fileset), "fileset_bin_files.nix")
...
def test_fileset_nested_directories_snapshot(snapshot):
    ...
    snapshot.assert_match(expr.dumps(fileset), "fileset_nested_dirs.nix")
...
def test_fileset_command_escaping_snapshot(snapshot):
    ...
    snapshot.assert_match(expr.dumps(fileset), "fileset_escaping.nix")


def test_fileset_custom_pkgs_snapshot(snapshot):
    """Test FileSet with custom pkgs parameter."""
    custom_pkgs = raw("myCustomNixpkgs")
    fileset = FileSet(
        paths={
    ...
    snapshot.assert_match(expr.dumps(fileset), "fileset_custom_pkgs.nix")
...
def test_store_path_integration():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        nix_expr = expr.dumps(store_path)
...
def test_fileset_integration():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        fileset_expr = expr.dumps(fileset)
